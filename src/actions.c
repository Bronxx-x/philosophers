...
/*   Updated: 2025/03/08 23:51:24 by yousef           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int starvation_ch(t_philo *philos)
{
    ...
    if (get_current_time_in_ms() - philos->last_meal > philos->die_t || *(philos->died) != -1)
    {
...
void    who_died(t_vars *vars)
{
    ...
    printf(" %ld ms, philo number %d died\n", get_current_time_in_ms() - vars->sim_start, vars->died);
